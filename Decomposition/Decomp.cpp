...
*  (4+5+6=15).
*
*  Helpful hints for the algorithm used here came from:
*  https://www.maa.org/sites/default/files/Pong-1-0750774.pdf
*/
...
void decompose(unsigned long input) {
  ...
  #pragma omp parallel for
  for (unsigned long length = 1, temp = input; length < length_max; temp -= length, length++) {
    if (temp % length == 0) {
...
void decompose(unsigned long long input) {
  ...
  #pragma omp parallel for
  for (unsigned long long length = 1, temp = input; length < length_max; temp -= length, length++) {
    if (temp % length == 0) {
